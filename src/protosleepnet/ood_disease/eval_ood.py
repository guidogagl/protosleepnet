"""Out-of-domain evaluation of all models on Alzheimer HC and AD.

Usage:
    python eval_ood.py --gpu_id 0 --models_dir /path/to/pretrained
"""
import argparse
import importlib
import json
...
MODELS_INFO = [
    # (name, load_type, model_id, n_channels, seq_len)
...
def voting_evaluate(model, dataset, seq_len, n_ch, device, zero_emg=False):
    """Voting evaluation across all subjects."""
    all_preds = []
    all_labels = []
    ...
    for spec in dataset._subjects:
        ...
        if n_ch == 3:
            ...
            if zero_emg:
                x[:, 2] = 0.0  # Zero out EMG (3rd channel)
        else:
...
def main():
    ...
    parser.add_argument("--zero_emg", action="store_true",
                        help="Zero out EMG channel for 3ch models")
    ...
    for subset in ["HC", "AD"]:
        print(f"\n{'=' * 80}")
        print(f" OUT-OF-DOMAIN EVAL: Alzheimer {subset}")
        print(f"{'=' * 80}")
        ...
        for model_name, load_type, model_id, n_ch, seq_len in MODELS_INFO:
            # Skip 1ch models when --zero_emg (only test 3ch)
            if args.zero_emg and n_ch == 1:
                continue
    ...
    if args.output_dir:
        ...
        out_path = os.path.join(args.output_dir, "ood_results.json")
